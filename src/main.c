...
static volatile boot_status_e s_boot_status = BOOT_INIT;
...
static int32_t boot_chip_lock_check(void)
{
    if(!is_chip_lock())
    {
        boot_warn("MCU chip is NOT secure.");
    }
    else
    {
        boot_notice("MCU chip is secure.");
    }
    go_to_next_step();
    return 0;
}
...
boot_handle_TB g_status_handTB[] = 
{
    ...
    {BOOT_JUMP_TO_APP,"jump_to_app",boot_jump_tp_system},
...
boot_status_e get_boot_status(void)
{
